...
function AdminUserTab() {
    ...
    const handleToggleActive = async (username, currentIsActive) => {
        if (username === currentAdminUser?.username) {
            setActionStatus(prev => ({ ...prev, [username]: { error: "Cannot deactivate your own account." } }));
            return;
        }
        const newActiveStatus = !currentIsActive;
        setActionStatus(prev => ({ ...prev, [username]: { loading: true } }));
        try {
            await adminService.setUserActiveStatus(username, newActiveStatus);
            setActionStatus(prev => ({ ...prev, [username]: { success: `Account ${newActiveStatus ? 'activated' : 'deactivated'}.` } }));
            // Refresh users list
            fetchUsers();
        } catch (error) {
            console.error(`Failed to toggle active status for ${username}:`, error);
            setActionStatus(prev => ({ ...prev, [username]: { error: error.response?.data?.detail || 'Status update failed.' } }));
        }
    };
                                ...
                                <tr key={u.id} className="hover:bg-bp-gray-light/50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        ...
                                        <button
                                            ...
                                            className={`text-xs ${u.is_active ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'} text-white py-1 px-2 rounded disabled:opacity-50`}
                                            title={u.username === currentAdminUser?.username ? 'Cannot deactivate self' : (u.is_active ? 'Désactiver' : 'Activer')}
                                        >
                                            {actionStatus[u.username]?.loading ? '...' : (u.is_active ? 'Désactiver' : 'Activer')}
                                        </button>
                                    </td>
                                </tr>
                            ))}
